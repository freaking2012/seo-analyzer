...
test('if document not exists base meta tags description & viewport, return ["This page without <meta name="description"> tag", "This page without <meta name="viewport"> tag"]', async t => {
  const options = { names: ['description', 'viewport'] };
  const result = await metaBaseRule(fakeDOM(), options);
  t.deepEqual(result, ['This page without <meta name="description"> tag', 'This page without <meta name="viewport"> tag']);
});
